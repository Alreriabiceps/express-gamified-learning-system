...
exports.joinLobby = async (req, res) => {
    try {
        ...
        if (lobby.status === 'in-progress') {
            const gameServer = req.app.get('gameServer');
            if (gameServer && gameServer.games && typeof gameServer.games.keys === 'function') {
                console.log('[LobbyController] Current gameServer.games keys:', Array.from(gameServer.games.keys()));
            }
            ...
            if (io) {
                setTimeout(() => {
                    ...
                    console.log('[LobbyController] Emitted game:start for lobby:', lobby._id.toString());
                }, 100);
            } else {
