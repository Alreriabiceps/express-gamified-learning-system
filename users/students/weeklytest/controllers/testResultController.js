...
const UserWeeklyAttempt = require("../../teamtest/models/userWeeklyAttemptModel");
...
const saveTestResult = async (req, res) => {
  try {
    ...
    if (existingResult) {
      ...
      return res.status(200).json({
        ...
        data: {
          ...
          totalPoints: leaderboardEntry
            ? leaderboardEntry.totalPoints
            : existingResult.pointsEarned,
    ...
    // Also reflect this solo completion in UserWeeklyAttempt to unify locks across solo/team
    try {
      await UserWeeklyAttempt.updateOne(
        { userId: studentId, weekId: weekScheduleId },
        {
          $setOnInsert: {
            mode: "solo",
          },
          $set: {
            completedAt: testResult.completedAt || new Date(),
            score: score,
          },
        },
        { upsert: true }
      ...
      // Unique index may already exist from team attempt; ignore
      console.warn("UserWeeklyAttempt upsert (solo) warning:", e?.message || e);
    }
    ...
    if (!leaderboardEntry) {
      ...
      // If a previous solo/team result exists, adjust leaderboard by the delta
      const existing = await TestResult.findOne({
        studentId,
        weekScheduleId,
      });
      const prevPoints = existing?.pointsEarned || 0;
      const delta = pointsEarned - prevPoints;
      leaderboardEntry.totalPoints += delta;
      leaderboardEntry.weeklyPoints += delta;
      leaderboardEntry.monthlyPoints += delta;
    }
...
const getTestResultsForStudent = async (req, res) => {
  try {
        ...
        results: await Promise.all(
          results.map(async (result) => {
            const populatedAnswers = await Promise.all(
              result.answers.map(async (ans) => {
                const questionDetails = await Question.findById(ans.questionId)
                  .select("questionText choices correctAnswer bloomsLevel")
                  .lean();
                return {
            ...
            return {
              ...
              title:
                result.weekScheduleId?.title ||
                `${result.subjectId?.subject || "Test"} - Week ${
                  result.weekScheduleId?.weekNumber || ""
                }`,
              score: result.score,
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch test results" });
  }
};
...
const getTestResultDetailsById = async (req, res) => {
  try {
    ...
    const responseData = {
      id: testResult._id,
      title:
        testResult.weekScheduleId?.title ||
        `${testResult.subjectId?.subject || "Test"} - Week ${
          testResult.weekScheduleId?.weekNumber || ""
        }`,
      subject: testResult.subjectId?.subject,
      student: {
        id: testResult.studentId?._id,
        name: `${testResult.studentId?.firstName || ""} ${
          testResult.studentId?.lastName || ""
        }`.trim(),
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch test result details" });
  }
};
...
const getStudyProgress = async (req, res) => {
  try {
    ...
    const weeklyScores = testResults
      .reverse()
      .map((result) =>
    ...
    const weakestSkill = Object.entries(bloomsAnalysis).reduce((a, b) =>
      bloomsAnalysis[a[0]] > bloomsAnalysis[b[0]] ? b : a
    );
...
const getAdaptiveLearning = async (req, res) => {
  try {
    ...
    const avgScore =
      recentResults.reduce((sum, r) => sum + r.score / r.totalQuestions, 0) /
      recentResults.length;
    const currentLevel =
      avgScore >= 0.8
        ? "Advanced"
