...
const refreshToken = async (req, res) => {
  try {
    // Accept either Authorization: Bearer <token> or { token } in body
    const authHeader = req.headers.authorization || "";
    const bodyToken = req.body?.token;
    const bearerToken = authHeader.startsWith("Bearer ")
      ? authHeader.split(" ")[1]
      : undefined;
    const oldToken = bodyToken || bearerToken;

    if (!oldToken) {
      return res.status(400).json({ error: "No token provided" });
    }

    // Decode without verifying exp to allow refresh when expired
    let decoded;
    try {
      decoded = jwt.verify(
        oldToken,
      ...
      // If token expired, decode payload anyway
      try {
...
const studentRegister = async (req, res) => {
  ...
  const timeout = setTimeout(() => {
    if (!res.headersSent) {
      console.error("Registration timeout - request took too long");
      res.status(408).json({
        error: "Registration timeout",
        message: "The registration process took too long. Please try again.",
      });
    }
  ...
  try {
    ...
    console.log("Validation passed - checking for existing students...");
    ...
    console.log("Existing student check:", {
      existingStudent: existingStudent ? "Found" : "Not found",
      existingPending: existingPending ? "Found" : "Not found",
    });
    ...
    console.log("No existing students found - creating new student...");

    // Create student directly (skip email verification and admin approval)
    const student = new Student({
      ...
      section,
      yearLevel,
      isEmailConfirmed: true, // Auto-confirm
      isApproved: true, // Auto-approve
      isActive: true,
    });

    console.log("Student object created, attempting to save...");
    await student.save();
